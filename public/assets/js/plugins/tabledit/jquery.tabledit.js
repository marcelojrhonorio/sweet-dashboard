...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
        ...
        function ajax(action)
        {
            ...
            if (action === 'delete') {
                serialize = {'id' : sweet.common.onlyNumber(serialize)};
            } else {
                serialize = sweet.common.querystringToHash(serialize);
            }

            //var jqXHR = $.post(settings.url, serialize, function(data, textStatus, jqXHR) {
            var jqXHR = $.post(settings.url.concat('/', action), serialize, function(data, textStatus, jqXHR) {
